  ...
  generate(value: string, options?: QROptions): android.graphics.Bitmap {
    ...
    if(options.logo) {
        try {
            ...
            const qrImg = net.glxn.qrgen.android.QRCode.from(value)
            ...
            const canvas: android.graphics.Canvas = new android.graphics.Canvas(qrImg);
            const resizedLogo = android.graphics.Bitmap.createScaledBitmap(
                logo, centerRatio.w, centerRatio.h, true);
            logo.recycle();
            canvas.drawBitmap(resizedLogo, centerRatio.x, centerRatio.y, null);
